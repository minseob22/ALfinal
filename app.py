...
def gcd(a, b):               #Implements the Euclidean algorithm to compute the greatest common divisor of two integers.
    while b != 0:
        a, b = b, a % b
    return a

def modinv(a, m):             #Uses the extended Euclidean algorithm to compute the modular inverse.
    m0, y, x = m, 0, 1
    if m == 1:
        return 0
    while a > 1:
...
def generate_keys(p, q):      #Generates RSA public and private keys using two prime numbers p & q
    n = p * q
    phi = (p - 1) * (q - 1)
...
def rsa_demo():
    ...
    return render_template_string('''
        ...
        <div class="container">
            ...
            <div>
                ...
                <p><strong>e (Public Exponent):</strong> {{ public_key[0] }}</p>
                <p><strong>d (Private Exponent):</strong> {{ private_key[0] }}</p>
            </div>
    ...
    ''', p=p, q=q, public_key=public_key, private_key=private_key, message=message, signature=signature,
    original_message_hash=original_message_hash, decrypted_signature_hash=decrypted_signature_hash, 
